...
export default function ExpenseImportDialog({ isOpen, onClose, onImport, userCategories }: ExpenseImportDialogProps) {
      ...
      complete: (results) => {
        ...
        results.data.forEach((row: any, index) => {
          ...
          if (!title || !amount || !date || !category) {
            invalidRows.push({ row: index + 2, data: row, error: "Missing required fields (title, amount, date, category)." });
            return;
          }

          const parsedAmount = parseFloat(amount);
          if (isNaN(parsedAmount) || parsedAmount <= 0) {
            invalidRows.push({ row: index + 2, data: row, error: `Invalid amount: ${amount}` });
            return;
          }

          const parsedDate = new Date(date);
          if (isNaN(parsedDate.getTime())) {
            invalidRows.push({ row: index + 2, data: row, error: `Invalid date format: ${date}. Use YYYY-MM-DD.` });
            return;
          }
  ...
  const downloadTemplate = () => {
    const csvContent = "data:text/csv;charset=utf-8," + "title,amount,date,category,status,recurrence,notes\n" + "Example Coffee,2.50,2023-10-27,Food,completed,one-time,Morning coffee";
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "xpns_import_template.csv");
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-3xl flex flex-col max-h-[90dvh]">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'upload' ? 'Upload a CSV file to bulk-import your expenses.' : 'Review your expenses before importing.'}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="py-4 space-y-6">
            <div className="space-y-2 p-4 border rounded-lg">
                ...
                <ol className="list-decimal list-inside text-sm text-muted-foreground space-y-1">
                    <li>Your CSV file must have the headers: `title`, `amount`, `date`, `category`.</li>
                    <li>Optional headers are: `status`, `recurrence`, `notes`.</li>
                    <li>The date format must be `YYYY-MM-DD`.</li>
                    <li>
                        <Button variant="link" onClick={downloadTemplate} className="p-0 h-auto">Download a template file</Button> to get started.
                    </li>
                </ol>
            </div>
            <div className="flex items-center justify-center w-full">
              <label htmlFor="csv-upload" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer bg-secondary/50 hover:bg-secondary">
            ...
            <div className="flex-1 overflow-hidden flex flex-col gap-4">
                ...
                {parsedData.validExpenses.length > 0 ? (
                    ...
                    <Alert>
                        ...
                        <AlertDescription>
                            We couldn't find any expenses to import. Please check your CSV file against the template and try again.
                        </AlertDescription>
                    </Alert>
                )}
            </div>
